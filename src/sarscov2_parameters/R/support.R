load_mcmc_parameters <- function(assumptions, deterministic,
  ...
  multi <- multiregion & !multi_pars_from_single
  path_pars <- file.path("pars", if (multi) "multi" else "single", assumptions,
                         if (deterministic) "deterministic" else "stochastic")
  ...
  if (multiregion) {
    info <- multiregion_info(info, fixed_pars)
    proposal <- multiregion_proposal(info, proposal)
    prior <- multiregion_prior(info, prior)
  }
